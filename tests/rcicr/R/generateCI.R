...
# Main function -----------------------------------------------------------
generateCI <- function(stimuli, responses, baseimage, rdata, rdatapath=FALSE, participants=NA,
  ...
  # NOTE: To accomplish equivalence testing, I have added the rdatapath variable
  # since we cannot directly load and pass Rdata files as variables.
  # So to circumvent this, if rdatapath is True, then the value of rdata is actually
  # a path that R can load within its own code circumventing loading Rdata within python
  # and then passing it as a variable as in the original R code.

  # Suppress X11 display when we run this in a docker container
  options(bitmapType='cairo') # Select non-X11 bitmap output device
  ...
  if (!rdatapath) {
    ...
    #load(file.path(getwd(), rdata))  # Assuming rdata contains the file name
    load(file.path(rdata))  # Assuming rdata contains the file name
  }
  ...
  if(zmapbool) {
    ...
    if (zmapmethod == 'quick') {
      zmap <- process_quick_zmap(ci, sigma, threshold, img_size)
    } else if (zmapmethod == 't.test') {
      zmap <- process_ttest_zmap(params, responses, p, img_size, ci, n_cores, pid.cis, participants)
    }
    # Pass zmap object to plotZmap for plotting
    plotZmap(zmap = zmap, bgimage = combined, filename = filename,
             sigma = sigma, threshold = threshold, size = img_size,
             targetpath = zmaptargetpath, decoration = zmapdecoration)
  }
...
### NOTE: The following two functions have been extracted from the source code, so that they can be tested in isolation
process_quick_zmap <- function(ci, sigma, threshold, img_size) {
  # Blur CI
  zmap <- as.matrix(blur(as.im(ci), sigma = sigma))
  # Create z-map
  zmap <- matrix(scale(as.vector(zmap)), img_size, img_size)
...
process_quick_zmap_step_by_step_asim <- function(ci){
  asim <- as.im(ci)
  return(asim=as.matrix(asim))
}

process_quick_zmap_step_by_step <- function(ci, sigma, threshold, img_size) {
  asim <- as.im(ci)
  # Blur CI
  blurred_image <- as.matrix(blur(asim, sigma = sigma))
  # Create z-map
  scaled_image <- matrix(scale(as.vector(blurred_image)), img_size, img_size)
  # Apply threshold
  zmap <- scaled_image
  zmap[zmap > -threshold & zmap < threshold] <- NA
  return(list(blurred_image=blurred_image, scaled_image=scaled_image,zmap=zmap))
}

process_ttest_zmap <- function(params, responses, p, img_size, ci, n_cores, pid.cis, participants) {
  if (all(is.na(participants))) {
    weightedparameters <- params * responses
    n_observations <- length(responses)
    pb <- txtProgressBar(min = 1, max = n_observations, style = 3)
...
process_ttest_zmap_sideloaded <- function(params, responses, img_size, ci, rdata, rdatapath=TRUE, participants=NA, n_cores = 1, pid.cis = NULL) {
  # Load parameter file (created when generating stimuli)
  if (!rdatapath) {
    # Load data as a variable
    load(rdata)
...
### NOTE: End of extracted functions
