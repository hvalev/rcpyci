...
def analyze_data(data: pd.DataFrame,
    ...
    def process_conditions(conditions, 
        ...
        # pass along input variables apart from already consumed ones
        kwargs = {k: v for k, v in locals().items()}
        consumed_variables = ['conditions', 'n_jobs', 'process_participant', 'process_condition']
        for key in consumed_variables:
            kwargs.pop(key, None)
    ...
    def process_participants(participants, 
        ...
        # pass along input variables apart from already consumed ones
        kwargs = {k: v for k, v in locals().items()}
        consumed_variables = ['participants', 'n_jobs', 'process_participant', 'process_condition']
        for key in consumed_variables:
            kwargs.pop(key, None)
        ...
        logging.info("Started calculating ci and zmaps for individual participants. "
                     "Be mindful that with higher parallel jobs the progress bar becomes more inaccurate. "
                     "This may take a while... ")
        result = Parallel(n_jobs=n_jobs)(delayed(process_participant)(
    ...
    # pass along input variables apart from already consumed ones
    kwargs = {k: v for k, v in locals().items()}
    consumed_variables = ['base_face_path', 'process_participants', 'process_participant', 'process_conditions', 'process_condition']
    for key in consumed_variables:
        kwargs.pop(key, None)
