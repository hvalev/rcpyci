...
generateStimuli2IFC <- function(base_face_files, overwrite_params=FALSE, n_trials=770, img_size=512, stimulus_path='./stimuli', label='rcic', use_same_parameters=TRUE, seed=1, maximize_baseimage_contrast=TRUE, noise_type='sinusoid', nscales=5, sigma=25, ncores=parallel::detectCores()-1, return_as_dataframe=FALSE, save_as_png=TRUE, save_rdata=TRUE) {
  ...
  if (use_same_parameters) {
    # NOTE: This is artificially added to allow
    # sideloading of params from a numpy array
    # to accomplish equivalence testing.
    if (is.logical(overwrite_params) && !overwrite_params) {
      ...
      cat("Generated Dimensions:", dim(params), "\n")
      cat("Generated Type:", class(params), "\n")
    } else {
      # Use overwrite_params directly
      params <- overwrite_params
      # Suppose 'my_array' is your array
      cat("Passed-in Dimensions:", dim(overwrite_params), "\n")
      cat("Passed-in Type:", class(overwrite_params), "\n")
  ...
  if (save_rdata) {
    #NOTE: For some reason, running this line results in the trial variable not being available in the context
    #the save call has been modified to not include it as it's not needed in generateCI 
    ...
    save(base_face_files, base_faces, img_size, label, n_trials, noise_type, p, seed, stimuli_params, stimulus_path, use_same_parameters, generator_version, file=paste(stimulus_path, paste(label, "seed", seed, "time", format(Sys.time(), format="%b_%d_%Y_%H_%M.Rdata"), sep="_"), sep='/'), envir=environment())
  }
